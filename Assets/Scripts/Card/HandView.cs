...
public class HandView : MonoBehaviour
{
    ...
    // 소멸 카드는 버린 카드 더미로 날아가지 않고 제자리에서 위로 뜨며 디졸브된다 (UIDissolve).
    [FormerlySerializedAs("exhaustNoise")]
    [SerializeField] private Texture _exhaustNoise; // 디졸브 노이즈 텍스처. 비우면 페이드로 대체.
    ...
    private float PlayHandExitAnimation()
    {
        ...
        for (int i = 0; i < _cardViews.Count; i++)
        {
            ...
            else if (IsExhausted(card))
            {
                PlayExhaust(view); // 턴 종료 시 소멸(휘발성 등)
            }
            else
            {
                Destroy(view.gameObject);
    ...
    private static bool IsExhausted(CardData card)
    {
        var exhaustPile = BattleManager.Instance != null ? BattleManager.Instance.State?.ExhaustPile : null;
        return card != null && exhaustPile != null && exhaustPile.Contains(card);
    }

    // 소멸 카드는 손패 컨테이너 밖으로 빼서 제자리에서 디졸브한 뒤 제거한다.
    private void PlayExhaust(CardView view)
    {
        GameObject go = view.gameObject; // 소멸 연출이 끝나면 제거할 카드 UI.
        foreach (var handler in go.GetComponents<CardHandler>()) handler.enabled = false; // 더 이상 드래그·호버 안 받게
        go.transform.SetParent(_cardContainer.parent != null ? _cardContainer.parent : transform, true);

        UIDissolve.Out(go, _exhaustNoise, _exhaustDuration, UIDissolve.DefaultEdge)
                  .Join(go.transform.DOBlendableLocalMoveBy(new Vector3(0f, 40f, 0f), _exhaustDuration))
